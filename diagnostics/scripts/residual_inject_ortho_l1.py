...
class ResidualInject(nn.Module):
    """Bilinear with orthogonal projection mode.
    L1: project R away from T_pool (new-class text direction), keeping old-class subspace safe.
    No extra params, no precomputation."""
    ...
    def _apply_ortho(self, R, T_pool):
        """L1: project R away from T_pool direction."""
        if self.ortho_mode != 'L1' or T_pool is None:
            return R
        v = T_pool.squeeze(0)  # (256,)
        vn = v.norm()
        if vn < 1e-4: return R
        v = v / vn
    ...
    def forward(self, memory, T_new=None):
        if self.mode == 'bilinear':
            ...
            residual = self._apply_ortho(residual, T_pool)
